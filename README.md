...
- **网页登录、授权登录与 API Key** —— 默认在内置 WebView 中登录；Claude 可选 OAuth 授权，Codex 可选浏览器设备码授权，手机无需安装 CLI。登录凭据通过额度校验后才保存；更换账号会提示确认。
- **额度时间与扩展指标** —— 保存绝对重置时间，首页按当前时间重算倒计时与时间比例；支持 Claude 额外用量百分比、Codex 附加限额及独立的额外额度。
...
| **Codex**（ChatGPT） | 网页登录 / 可选设备码授权 | 按账户计划自动识别 | 5 小时限额、周限额、附加限额、Code Review、续订 / 到期时间、重置卡、额外额度 |
...
**可选授权登录：** 在 Claude 登录页选择「授权登录」，或在 Codex 登录页选择「设备码登录」，按页面指引完成授权。授权失败可返回网页登录，原有登录不会因未完成的新授权而被覆盖。旧版本尚未保存真实账号身份时，首次重新登录也会提示替换确认。OAuth 凭据加密保存，临期自动续期；这类接入依赖平台当前授权流程，真实账号授权仍应在设备上验证。
